...
import { partnersApi, HomepagePartnerItem } from "../../services";
...
const PartnersSection: React.FC<PartnersSectionProps> = ({
  ...
  const [partners, setPartners] =
    useState<HomepagePartnerItem[]>(initialPartners);
  ...
  useEffect(() => {
    if (initialPartners && initialPartners.length > 0) {
      const sorted = [...initialPartners].sort(
  ...
  const totalSlides = useMemo(
    () => Math.max(1, Math.ceil(partners.length / partnersPerPage)),
  ...
  const handleScroll = useCallback(() => {
    ...
    const newSlide = Math.round(
      scrollLeft / (cardWidthWithGap * partnersPerPage)
    );
            ...
            {partners.map((partner) => (
              <div key={partner.id} className={styles.partners__card}>
                <div className={styles.partners__logoWrapper}>
                  <Image
                    src={partnersApi.buildLogoUrl(
                      partner.logo?.formats?.thumbnail?.url || partner.logo?.url
                    )}
                    alt={partner.name}
                    width={200}
                    height={120}
